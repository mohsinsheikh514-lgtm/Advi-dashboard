...
import {
  ...
  fetchAgeGenderBreakdown, fetchCampaignAdsets,
...
export default function Home() {
  ...
  const loadAccount = useCallback(async (acc) => {
    ...
    try {
      const [ins, prevIns, camps, monthly, ads, daily, funnel, placements] = await Promise.all([
          ...
          <>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20, flexWrap: "wrap", gap: 10 }}>
              ...
              <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                ...
                <div style={{ display: "flex", gap: 3, background: T.card, borderRadius: 8, padding: 3 }}>
                  {tabs.map(t => (
                    <button key={t.id} onClick={() => setTab(t.id)}
                      style={{ padding: "7px 14px", borderRadius: 6, border: "none", background: tab === t.id ? T.accent : "transparent", color: tab === t.id ? "#fff" : T.textMuted, fontSize: 11, fontWeight: 500, cursor: "pointer" }}>{t.label}</button>
                  ))}
                </div>
              </div>
            </div>
